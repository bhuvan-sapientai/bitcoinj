...
public class HDKeyDerivationSapientGeneratedJunit4Test {
    ...
    public void deriveThisOrNextChildKeyWhenCaughtHDDerivationExceptionThrowsHDDerivationException() {
        ...
        try (MockedStatic<HDKeyDerivation> hDKeyDerivation = mockStatic(HDKeyDerivation.class, CALLS_REAL_METHODS)) {
            hDKeyDerivation.when(() -> HDKeyDerivation.deriveChildKey(eq(deterministicKeyMock2), (ChildNumber) any())).thenThrow(hDDerivationExceptionMock);
            ...
            thrown.expectMessage(hDDerivationException.getMessage());
            //Act Statement(s)
            HDKeyDerivation.deriveThisOrNextChildKey(deterministicKeyMock2, 5);
            //Assert statement(s)
    ...
    public void deriveChildKeyFromPrivateTest() throws HDDerivationException {
        ...
        try (MockedStatic<ByteUtils> byteUtils = mockStatic(ByteUtils.class);
            ...
            byte[] byteArray = new byte[]{};
            byteUtils.when(() -> ByteUtils.bytesToBigInteger(byteArray)).thenReturn(new BigInteger("16909060"));
            //Act Statement(s)
            ...
            byte[] byteArray2 = new byte[]{};
            DeterministicKey deterministicKey = new DeterministicKey(hDPath2, byteArray2, new BigInteger("16909060"), parentMock);
            //Assert statement(s)
            assertThat(result, equalTo(deterministicKey));
    ...
    public void deriveChildKeyFromPublicTest() throws HDDerivationException {
        ...
        try (MockedStatic<HDKeyDerivation> hDKeyDerivation = mockStatic(HDKeyDerivation.class, CALLS_REAL_METHODS)) {
            ...
            hDKeyDerivation.when(() -> HDKeyDerivation.deriveChildKeyBytesFromPublic(parentMock, childNumber, HDKeyDerivation.PublicDeriveMode.NORMAL)).thenReturn(hDKeyDerivationRawKeyBytesMock);
            //Act Statement(s)
            DeterministicKey result = HDKeyDerivation.deriveChildKeyFromPublic(parentMock, childNumber, HDKeyDerivation.PublicDeriveMode.NORMAL);
            List list2 = new ArrayList<>();
            HDPath hDPath2 = new HDPath(false, list2);
            byte[] byteArray = new byte[]{};
            ECDomainParameters eCDomainParameters = ECKey.CURVE;
            ...
            assertThat(result, equalTo(deterministicKey));
            verify(parentMock, atLeast(1)).getPath();
            hDKeyDerivation.verify(() -> HDKeyDerivation.deriveChildKeyBytesFromPublic(parentMock, childNumber, HDKeyDerivation.PublicDeriveMode.NORMAL), atLeast(1));
        }
    }

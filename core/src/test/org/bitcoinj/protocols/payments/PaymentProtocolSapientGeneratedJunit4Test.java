...
public class PaymentProtocolSapientGeneratedJunit4Test {
    ...
    public void createPaymentRequestTest() {
        ...
        try (MockedStatic<PaymentProtocol> paymentProtocol = mockStatic(PaymentProtocol.class, CALLS_REAL_METHODS)) {
            ...
            paymentProtocol.when(() -> PaymentProtocol.createPayToAddressOutput((Coin) any(), eq(addressMock))).thenReturn(output);
            byte[] byteArray = new byte[]{(byte) 0};
            paymentProtocol.when(() -> PaymentProtocol.createPaymentRequest((NetworkParameters) any(), anyList(), eq("String"), eq("String"), eq(byteArray))).thenReturn(protosPaymentRequestBuilderMock);
            NetworkParameters networkParameters = NetworkParameters.fromID("id1");
            Coin coin = Coin.valueOf(0L);
            //Act Statement(s)
            Protos.PaymentRequest.Builder result = PaymentProtocol.createPaymentRequest(networkParameters, coin, addressMock, "String", "String", byteArray);
            //Assert statement(s)
            assertThat(result, equalTo(protosPaymentRequestBuilderMock));
            paymentProtocol.verify(() -> PaymentProtocol.createPayToAddressOutput((Coin) any(), eq(addressMock)), atLeast(1));
            paymentProtocol.verify(() -> PaymentProtocol.createPaymentRequest((NetworkParameters) any(), anyList(), eq("String"), eq("String"), eq(byteArray)), atLeast(1));
        }
    }
    ...
    public void createPaymentRequest1WhenMerchantDataIsNotNull() {
        ...
        try (MockedStatic<Protos.PaymentRequest> protosPaymentRequest = mockStatic(Protos.PaymentRequest.class);
            ...
            doReturn(protosPaymentDetailsBuilderMock6).when(protosPaymentDetailsBuilderMock).setTime(1703143213L);
            ...
            verify(protosPaymentDetailsBuilderMock, atLeast(1)).setTime(1703143213L);
    ...
    public void createPaymentMessageWhenRefundAmountIsNotNull() {
        ...
        try (MockedStatic<PaymentProtocol> paymentProtocol = mockStatic(PaymentProtocol.class, CALLS_REAL_METHODS)) {
            ...
            byte[] byteArray = new byte[]{};
            paymentProtocol.when(() -> PaymentProtocol.createPaymentMessage(anyList(), anyList(), eq(""), eq(byteArray))).thenReturn(payment);
            Transaction transaction = new Transaction();
            List<Transaction> transactionList = new ArrayList<>();
            transactionList.add(transaction);
            //Act Statement(s)
            Protos.Payment result = PaymentProtocol.createPaymentMessage(transactionList, coinMock, addressMock, "", byteArray);
            //Assert statement(s)
            ...
            paymentProtocol.verify(() -> PaymentProtocol.createPaymentMessage(anyList(), anyList(), eq(""), eq(byteArray)), atLeast(1));
        }
    }

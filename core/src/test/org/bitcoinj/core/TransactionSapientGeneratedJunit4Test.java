...
public class TransactionSapientGeneratedJunit4Test {
    ...
    byteUtils.when(() -> ByteUtils.readUint32((ByteBuffer) any())).thenReturn(0L).thenReturn(16909060L);
    LockTime lockTime2 = LockTime.of(0L);
    lockTime.when(() -> LockTime.of(16909060L)).thenReturn(lockTime2);
    ByteBuffer byteBuffer = ByteBuffer.allocateDirect(0);
    //Act Statement(s)
    Transaction result = Transaction.read(byteBuffer, 70015);
    //Assert statement(s)
    assertThat(result, is(notNullValue()));
    byteUtils.verify(() -> ByteUtils.readUint32((ByteBuffer) any()), atLeast(2));
    lockTime.verify(() -> LockTime.of(16909060L), atLeast(1));
}*/
    }
    ...
    public void addOutput3Test() {
        ...
        try (MockedStatic<LockTime> lockTime = mockStatic(LockTime.class)) {
            ...
            Script script = Script.of(list);
            //Act Statement(s)
            TransactionOutput result = target.addOutput(coinMock, script);
            byte[] byteArray = new byte[]{};

...
public class RejectMessageSapientGeneratedJunit4Test {
    ...
    public void readWhenMessageEqualsTx() throws BufferUnderflowException, ProtocolException {
        ...
        try (MockedStatic<Sha256Hash> sha256Hash = mockStatic(Sha256Hash.class);
            ...
            rejectMessageRejectCode.when(() -> RejectMessage.RejectCode.fromCode((byte) 0)).thenReturn(RejectMessage.RejectCode.INVALID);
            ...
            RejectMessage rejectMessage = new RejectMessage(RejectMessage.RejectCode.INVALID, sha256Hash2, "tx", "block");
            //Assert statement(s)
    ...
    buffers.when(() -> Buffers.readLengthPrefixedString((ByteBuffer) any())).thenReturn("").thenReturn("Hello World");
    ByteBuffer byteBuffer = ByteBuffer.allocateDirect(0);
    //Act Statement(s)
    RejectMessage result = RejectMessage.read(byteBuffer);
    RejectMessage rejectMessage = new RejectMessage(RejectMessage.RejectCode.MALFORMED, (Sha256Hash) null, "", "Hello World");
    //Assert statement(s)
    assertThat(result, equalTo(rejectMessage));
    buffers.verify(() -> Buffers.readLengthPrefixedString((ByteBuffer) any()), atLeast(2));

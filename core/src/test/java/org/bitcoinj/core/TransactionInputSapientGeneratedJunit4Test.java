...
public class TransactionInputSapientGeneratedJunit4Test {
    ...
    public void verify1WhenGetOutpointIndexNotEqualsOutputGetIndexThrowsVerificationException() throws VerificationException, ScriptException {
        ...
        //TransactionInput target = spy(new TransactionInput(transactionMock, byteArray, outpointMock, coin));
        ...
        //doNothing().when(scriptMock2).correctlySpends(eq(transactionMock), eq(2), eq(transactionWitness), (Coin) any(), eq(scriptMock), eq(scriptVerifyFlagSet));
        ...
        //verify(scriptMock2).correctlySpends(eq(transactionMock), eq(2), eq(transactionWitness), (Coin) any(), eq(scriptMock), eq(scriptVerifyFlagSet));
    ...
    public void verify1WhenGetOutpointIndexEqualsOutputGetIndexAndWitnessIsNull() throws VerificationException, ScriptException {
        ...
        ByteBuffer byteBuffer = ByteBuffer.allocateDirect(0);
        Sha256Hash sha256Hash = Sha256Hash.read(byteBuffer);
        doReturn(sha256Hash).when(transactionMock).getTxId();
        ...
        doReturn(sha256Hash2).when(transactionOutPointMock).hash();
        TransactionOutPoint transactionOutPointMock2 = mock(TransactionOutPoint.class);
        doReturn(transactionOutPointMock, transactionOutPointMock2).when(target).getOutpoint();
        doReturn(1L).when(transactionOutPointMock2).index();
        Script scriptMock = mock(Script.class);
        ...
        verify(outputMock).getScriptPubKey();
        verify(target, times(2)).getOutpoint();
        verify(transactionOutPointMock).hash();
        verify(transactionOutPointMock2).index();
        verify(target).getScriptSig();
    ...
    public void toStringWhenCaughtScriptException() throws ScriptException {
        ...
        //TransactionInput target = spy(new TransactionInput(transactionMock, byteArray, outpointMock, coinMock));
        //doReturn(false).when(target).isCoinBase();
        //ScriptException scriptException = new ScriptException(ScriptError.SCRIPT_ERR_OK, "msg1");
        //scriptException.setMessage("A");

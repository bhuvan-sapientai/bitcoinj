...
public class BloomFilterSapientGeneratedJunit4Test {
    ...
    public void getFalsePositiveRateTest() {
        //Arrange Statement(s)
        BloomFilter target = new BloomFilter(10, Double.parseDouble("0.1"), 5, BloomFilter.BloomUpdate.UPDATE_NONE);

        //Act Statement(s)
        double result = target.getFalsePositiveRate(10);

        //Assert statement(s)
        assertThat(result, closeTo(Double.parseDouble("0.15481812174617549"), 0.00001));
    }
    ...
    public void toStringTest() {
        //Arrange Statement(s)
        BloomFilter target = spy(new BloomFilter(10, Double.parseDouble("0.01"), 5, BloomFilter.BloomUpdate.UPDATE_ALL));
    ...
    public void insertWhenILessThanHashFuncs() {
        ...
        try (MockedStatic<ByteUtils> byteUtils = mockStatic(ByteUtils.class);
             MockedStatic<BloomFilter> bloomFilter = mockStatic(BloomFilter.class)) {
            byte[] byteArray = new byte[]{(byte) 1, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0};
            byte[] byteArray2 = new byte[]{(byte) 0};
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 5L, 0, byteArray2)).thenReturn(0);
            byteUtils.when(() -> ByteUtils.setBitLE(byteArray, 0)).thenAnswer((Answer<Void>) invocation -> null);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 5L, 1, byteArray2)).thenReturn(1);
            byteUtils.when(() -> ByteUtils.setBitLE(byteArray, 1)).thenAnswer((Answer<Void>) invocation -> null);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 5L, 2, byteArray2)).thenReturn(1);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 5L, 3, byteArray2)).thenReturn(1);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 5L, 4, byteArray2)).thenReturn(1);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 5L, 5, byteArray2)).thenReturn(1);
            BloomFilter target = new BloomFilter(10, Double.parseDouble("0.01"), 5, BloomFilter.BloomUpdate.UPDATE_NONE);
            //Act Statement(s)
            target.insert(byteArray2);
            //Assert statement(s)
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 5L, 0, byteArray2), atLeast(1));
            byteUtils.verify(() -> ByteUtils.setBitLE(byteArray, 0), atLeast(1));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 5L, 1, byteArray2), atLeast(1));
            byteUtils.verify(() -> ByteUtils.setBitLE(byteArray, 1), atLeast(5));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 5L, 2, byteArray2), atLeast(1));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 5L, 3, byteArray2), atLeast(1));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 5L, 4, byteArray2), atLeast(1));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 5L, 5, byteArray2), atLeast(1));
        }
    }
    ...
    public void setMatchAllTest() {
        //Arrange Statement(s)
        BloomFilter target = new BloomFilter(10, Double.parseDouble("0.01"), 5, BloomFilter.BloomUpdate.UPDATE_ALL);

...
public class BloomFilterSapientGeneratedJunit4Test {
    ...
    public void getFalsePositiveRateTest() {
        //Arrange Statement(s)
        BloomFilter target = new BloomFilter(100, Double.parseDouble("0.01"), 0, BloomFilter.BloomUpdate.UPDATE_ALL);
        //Act Statement(s)
        double result = target.getFalsePositiveRate(100);
        //Assert statement(s)
        assertThat(result, closeTo(Double.parseDouble("0.010445429053639082"), 0.00001));
    }
    ...
    public void toStringTest() {
        //Arrange Statement(s)
        BloomFilter target = spy(new BloomFilter(10, Double.parseDouble("0.01"), 123456, BloomFilter.BloomUpdate.UPDATE_ALL));
        doReturn(BloomFilter.BloomUpdate.UPDATE_P2PUBKEY_ONLY).when(target).getUpdateFlag();
        //Act Statement(s)
        String result = target.toString();
        //Assert statement(s)
        assertThat(result, equalTo("BloomFilter{data length=11, hashFuncs=6, nFlags=UPDATE_P2PUBKEY_ONLY}"));
    ...
    byteUtils.when(() -> ByteUtils.writeInt32LE(eq(123456), (OutputStream) any())).thenAnswer((Answer<Void>) invocation -> null);
    BloomFilter target = new BloomFilter(10, Double.parseDouble("0.01"), 123456, BloomFilter.BloomUpdate.UPDATE_ALL);
    ...
    byteUtils.verify(() -> ByteUtils.writeInt32LE(eq(123456), (OutputStream) any()));
}*/
    }
    ...
    public void containsWhenByteUtilsNotCheckBitLEDataMurmurHash3DataNTweakIObject() {
        ...
        try (MockedStatic<ByteUtils> byteUtils = mockStatic(ByteUtils.class);
            ...
            BloomFilter target = new BloomFilter(10, Double.parseDouble("0.01"), 0, BloomFilter.BloomUpdate.UPDATE_NONE);
            //Act Statement(s)
    ...
    public void insertWhenILessThanHashFuncs() {
        ...
        try (MockedStatic<ByteUtils> byteUtils = mockStatic(ByteUtils.class);
            ...
            byte[] byteArray = new byte[]{(byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0};
            byte[] byteArray2 = new byte[]{(byte) 0};
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 123456L, 0, byteArray2)).thenReturn(1);
            byteUtils.when(() -> ByteUtils.setBitLE(byteArray, 1)).thenAnswer((Answer<Void>) invocation -> null);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 123456L, 1, byteArray2)).thenReturn(1);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 123456L, 2, byteArray2)).thenReturn(1);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 123456L, 3, byteArray2)).thenReturn(1);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 123456L, 4, byteArray2)).thenReturn(1);
            bloomFilter.when(() -> BloomFilter.murmurHash3(byteArray, 123456L, 5, byteArray2)).thenReturn(1);
            BloomFilter target = new BloomFilter(10, Double.parseDouble("0.01"), 123456, BloomFilter.BloomUpdate.UPDATE_ALL);
            //Act Statement(s)
            target.insert(byteArray2);
            //Assert statement(s)
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 123456L, 0, byteArray2), atLeast(1));
            byteUtils.verify(() -> ByteUtils.setBitLE(byteArray, 1), atLeast(6));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 123456L, 1, byteArray2), atLeast(1));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 123456L, 2, byteArray2), atLeast(1));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 123456L, 3, byteArray2), atLeast(1));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 123456L, 4, byteArray2), atLeast(1));
            bloomFilter.verify(() -> BloomFilter.murmurHash3(byteArray, 123456L, 5, byteArray2), atLeast(1));
        }
    }
    ...
    public void insert1Test() {
        //Arrange Statement(s)
        ECKey keyMock = mock(ECKey.class, "byte array placeholder");
        byte[] byteArray = new byte[]{};
        doReturn(byteArray).when(keyMock).getPubKey();
        byte[] byteArray2 = new byte[]{};
        doReturn(byteArray2).when(keyMock).getPubKeyHash();
        BloomFilter target = spy(new BloomFilter(10, Double.parseDouble("0.01"), 0, BloomFilter.BloomUpdate.UPDATE_ALL));

...
public class VersionMessageSapientGeneratedJunit4Test {
    ...
    target = new VersionMessage(networkParametersMock, 1);
    autoCloseableMocks = MockitoAnnotations.openMocks(this);
    ...
    //TODO: Needs to return real value
    timeUtils.when(() -> TimeUtils.currentTime()).thenReturn(null);
    timeUtils.when(() -> TimeUtils.dateTimeFormat((Instant) null)).thenReturn("<current time in custom date-time format>");
    ...
    //Assert statement(s)
    assertThat(result, equalTo("\nclient version: 70013\n\ntime:           <current time in custom date-time format>\nreceiving svc:  null\nreceiving addr: null\nsub version:    null\nbest height:    0\ndelay tx relay: true\n"));
    timeUtils.verify(() -> TimeUtils.currentTime(), atLeast(1));

...
public class MultiplexingDiscoverySapientGeneratedJunit4Test {
    ...
    private final MultiplexingDiscovery multiplexingDiscoveryMock = mock(MultiplexingDiscovery.class, "UnknownObjectContent{target='org.bitcoinj.net.discovery.MultiplexingDiscovery', onlyPojoFunctions=false, builderPattern=false}");
    ...
    public void forServices1WhenDnsSeedsIsNotEmptyAndDefaultBranch() {
        ...
        try (MockedStatic<NetworkParameters> networkParameters = mockStatic(NetworkParameters.class)) {
            ...
            //Act Statement(s)
            MultiplexingDiscovery result = MultiplexingDiscovery.forServices(networkMock, 0L, true, false);
            //Assert statement(s)
    ...
    public void forServices2Test() {
        //Arrange Statement(s)
        try (MockedStatic<MultiplexingDiscovery> multiplexingDiscovery = mockStatic(MultiplexingDiscovery.class, CALLS_REAL_METHODS)) {
            multiplexingDiscovery.when(() -> MultiplexingDiscovery.forServices(networkMock, 1234567890L)).thenReturn(multiplexingDiscoveryMock);
            NetworkParameters networkParameters = NetworkParameters.fromID("id1");
            //Act Statement(s)
            MultiplexingDiscovery result = MultiplexingDiscovery.forServices(networkParameters, 1234567890L);
            //Assert statement(s)
            assertThat(result, equalTo(multiplexingDiscoveryMock));
            multiplexingDiscovery.verify(() -> MultiplexingDiscovery.forServices(networkMock, 1234567890L), atLeast(1));
        }
    }
    ...
    public void forServices3Test() {
        ...
        try (MockedStatic<MultiplexingDiscovery> multiplexingDiscovery = mockStatic(MultiplexingDiscovery.class, CALLS_REAL_METHODS)) {
            ...
            NetworkParameters networkParameters = NetworkParameters.fromID("id1");
            //Act Statement(s)
            MultiplexingDiscovery result = MultiplexingDiscovery.forServices(networkParameters, 0L, false, false);
            //Assert statement(s)
            assertThat(result, equalTo(multiplexingDiscoveryMock));
    ...
    public void getPeersWhenShufflePeers() throws PeerDiscoveryException {
        ...
        //TODO: Needs to return real value
        doReturn(null).when(target).createExecutor();
        PeerDiscoveryException peerDiscoveryException = new PeerDiscoveryException("No peer discovery returned any results in 10000 ms. Check internet connection?");
        thrown.expect(PeerDiscoveryException.class);
        thrown.expectMessage(peerDiscoveryException.getMessage());
    ...
    public void createExecutorTest() {
        ...
        PeerDiscovery peerDiscoveryMock3 = mock(PeerDiscovery.class, "8333");
        List<PeerDiscovery> peerDiscoveryList = new ArrayList<>();
        peerDiscoveryList.add(peerDiscoveryMock);
        peerDiscoveryList.add(peerDiscoveryMock2);
        peerDiscoveryList.add(peerDiscoveryMock3);
        ...
        ExecutorService executorService = Executors.newFixedThreadPool(3, contextPropagatingThreadFactory);

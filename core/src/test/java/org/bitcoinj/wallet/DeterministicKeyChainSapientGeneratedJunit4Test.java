...
public class DeterministicKeyChainSapientGeneratedJunit4Test {
    ...
    ChildNumber childNumber = new ChildNumber(0, false);
    ChildNumber[][] childNumberArray3 = new ChildNumber[][] { childNumber };
    ChildNumber childNumber2 = new ChildNumber(0, false);
    ChildNumber[][] childNumberArray4 = new ChildNumber[][] { childNumber2 };
    ChildNumber childNumber3 = new ChildNumber(0, false);
    ChildNumber[][] childNumberArray5 = new ChildNumber[][] { childNumber3 };
    ChildNumber[] childNumberArray2 = new ChildNumber[] { childNumberArray3, childNumberArray4, childNumberArray5 };
    ChildNumber childNumber4 = new ChildNumber(0, false);
    ChildNumber[][] childNumberArray7 = new ChildNumber[][] { childNumber4 };
    ChildNumber childNumber5 = new ChildNumber(0, false);
    ChildNumber[][] childNumberArray8 = new ChildNumber[][] { childNumber5 };
    ChildNumber childNumber6 = new ChildNumber(0, false);
    ChildNumber[][] childNumberArray9 = new ChildNumber[][] { childNumber6 };
    ChildNumber[] childNumberArray6 = new ChildNumber[] { childNumberArray7, childNumberArray8, childNumberArray9 };
    ChildNumber[] childNumberArray = new ChildNumber[] { childNumberArray2, childNumberArray6 };
    //Act Statement(s)
    ...
    public void serializeMyselfToProtobufWhenEntriesIsEmptyAndIsFollowingAndKeyGetParentIsNull() {
        ...
        HDPath hDPathMock = mock(HDPath.class);
        Protos.Key keyMock2 = mock(Protos.Key.class);
        try (MockedStatic<Protos.Key.OutputScriptType> protosKeyOutputScriptType = mockStatic(Protos.Key.OutputScriptType.class);
             MockedStatic<Preconditions> preconditions = mockStatic(Preconditions.class);
             MockedStatic<Threading> threading = mockStatic(Threading.class)) {
            doReturn(false).when(keyMock).isPubKeyOnly();
            doReturn(hDPathMock).when(keyMock).getPath();
            ...
            preconditions.when(() -> Preconditions.checkArgument(eq(false), (Supplier) any())).thenAnswer((Answer<Void>) invocation -> null);
            preconditions.when(() -> Preconditions.checkArgument(eq(true), (Supplier) any())).thenAnswer((Answer<Void>) invocation -> null);
            protosKeyOutputScriptType.when(() -> Protos.Key.OutputScriptType.valueOf("P2PKH")).thenReturn(Protos.Key.OutputScriptType.P2PKH);
            target = spy(new DeterministicKeyChain(keyMock, true, true, ScriptType.P2PKH));
            autoCloseableMocks = MockitoAnnotations.openMocks(this);
            doReturn(true).when(target).isFollowing();
            //Act Statement(s)
            ...
            preconditions.verify(() -> Preconditions.checkArgument(eq(false), (Supplier) any()));
            preconditions.verify(() -> Preconditions.checkArgument(eq(true), (Supplier) any()));
            protosKeyOutputScriptType.verify(() -> Protos.Key.OutputScriptType.valueOf("P2PKH"), atLeast(1));
            verify(target).isFollowing();
        }
    }
    ...
    List list = new ArrayList<>();
    HDPath hDPath = new HDPath(false, list);
    doReturn(deterministicKeyMock).when(hierarchyMock2).get(hDPath, false, false);
    ReentrantLock reentrantLock = Threading.lock(DeterministicKeyChain.class);
    preconditions.when(() -> Preconditions.checkArgument(eq(false), (Supplier) any())).thenAnswer((Answer<Void>) invocation -> null);
    preconditions.when(() -> Preconditions.checkArgument(eq(true), (Supplier) any())).thenAnswer((Answer<Void>) invocation -> null);
    //TODO: Needs to return real value
    ...
    doReturn(deterministicSeed).when(seedMock).encrypt(keyCrypterMock, aesKeyMock);
    ...
    doReturn(deterministicKeyMock3).when(hierarchyMock).get(hDPath2, false, false);
    ...
    HDPath hDPath3 = new HDPath(false, list3);
    doReturn(deterministicKeyMock5).when(hierarchyMock).get(hDPath3, false, false);
    ...
    doReturn(deterministicKeyMock7).when(hierarchyMock).get((HDPath) any(), eq(false), eq(false));
    ...
    HDPath hDPath4 = new HDPath(false, list4);
    doReturn(deterministicKeyMock9).when(hierarchyMock).get(hDPath4, false, false);
    ...
    listenerRegistrationKeyChainEventListenerList.add(listenerRegistration);
    ...
    verify(keyMock).isPubKeyOnly();
    verify(keyMock).getPath();
    verify(hierarchyMock2).get(hDPath, false, false);
    ...
    verify(hierarchyMock).get(hDPath2, false, false);
    ...
    verify(hierarchyMock).get(hDPath3, false, false);
    ...
    verify(hierarchyMock).get((HDPath) any(), eq(false), eq(false));
    ...
    verify(hierarchyMock).get(hDPath4, false, false);
    ...
    public void toEncrypted1WhenDefaultBranchAndDefaultBranchAndDefaultBranchAndDefaultBranch6() throws KeyCrypterException {
        ...
        try (MockedStatic<Preconditions> preconditions = mockStatic(Preconditions.class);
            ...
            doReturn(deterministicKeyMock).when(rootKeyMock).encrypt(keyCrypterMock, aesKeyMock, null);
            doReturn(deterministicKeyMock2).when(hierarchyMock).get(hDPathMock2, false, false);
            ...
            verify(seedMock).encrypt(keyCrypterMock, aesKeyMock);
            verify(rootKeyMock).encrypt(keyCrypterMock, aesKeyMock, null);
            verify(hierarchyMock).get(hDPathMock2, false, false);
        }
    }
    ...
    public void setLookaheadThresholdWhenNumGreaterThanOrEqualsToLookaheadSizeThrowsIllegalArgumentException() {
        ...
        try (MockedStatic<Preconditions> preconditions = mockStatic(Preconditions.class);
            ...
            autoCloseableMocks = MockitoAnnotations.openMocks(this);
            //Act Statement(s)
            target.setLookaheadThreshold(7);
            //Assert statement(s)
            verify(keyMock).isPubKeyOnly();
            verify(keyMock).getPath();
            threading.verify(() -> Threading.lock(DeterministicKeyChain.class), atLeast(1));
    ...
    public void getKeys1WhenNotIncludeLookahead() {
        ...
        try (MockedStatic<StreamUtils> streamUtils = mockStatic(StreamUtils.class);
            ...
            List list = new ArrayList<>();
            HDPath hDPath = new HDPath(false, list);
            doReturn(hDPath).when(internalParentKeyMock).getPath();
            //Act Statement(s)
    ...
    public void toString1WhenWatchingKeyCreationTimeIsPresent() {
        ...
        try (MockedStatic<TimeUtils> timeUtils = mockStatic(TimeUtils.class);
            ...
            //Assert statement(s)
            assertThat(result, equalTo("Key birthday:      1703224075  [String]\nOuput script type: P2TR\nKey to watch:      String\nLookahead siz/thr: 100/33\n"));
            verify(keyMock).isPubKeyOnly();
            verify(keyMock).getPath();
            threading.verify(() -> Threading.lock(DeterministicKeyChain.class), atLeast(1));
            preconditions.verify(() -> Preconditions.checkArgument(eq(false), (Supplier) any()));
    ...
    public void toString1WhenWatchingKeyCreationTimeNotIsPresent() {
        ...
        try (MockedStatic<TimeUtils> timeUtils = mockStatic(TimeUtils.class);
            ...
            String result = target.toString(false, false, aesKeyMock, networkMock);
            //Assert statement(s)
            assertThat(result, equalTo("Key birthday:      1703224076  [String]\nOuput script type: P2TR\nKey to watch:      A\nLookahead siz/thr: 100/33\n"));
            verify(keyMock).isPubKeyOnly();
            verify(keyMock).getPath();
            threading.verify(() -> Threading.lock(DeterministicKeyChain.class), atLeast(1));
            preconditions.verify(() -> Preconditions.checkArgument(eq(false), (Supplier) any()));
    ...
    ChildNumber childNumber = new ChildNumber(0, false);
    doReturn(childNumber).when(deterministicKeyMock).getChildNumber();
    StringBuilder stringBuilder = new StringBuilder();

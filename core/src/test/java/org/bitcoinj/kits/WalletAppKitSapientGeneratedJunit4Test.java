...
public class WalletAppKitSapientGeneratedJunit4Test {
    ...
    preconditions.when(() -> Preconditions.checkState(eq(false), (Supplier) any())).thenThrow(illegalStateException);
    File file = new File("pathname1");
    target = new WalletAppKit(paramsMock2, ScriptType.P2PKH, keyChainGroupStructureMock, file, "wallet");
    autoCloseableMocks = MockitoAnnotations.openMocks(this);
    thrown.expect(IllegalStateException.class);
    ...
    public void setAutoSaveWhenStateEqualsStateNEWThrowsIllegalStateException() {
         ...
         */
        //Arrange Statement(s)
        //NetworkParameters paramsMock2 = mock(NetworkParameters.class, "org.bitcoinj.params.MainNetParams");
        //KeyChainGroupStructure keyChainGroupStructureMock = mock(KeyChainGroupStructure.class, "DEFAULT");
        /*try (MockedStatic<Preconditions> preconditions = mockStatic(Preconditions.class)) {
    doReturn(null).when(paramsMock2).network();
    IllegalStateException illegalStateException = new IllegalStateException();
    preconditions.when(() -> Preconditions.checkState(eq(false), (Supplier) any())).thenThrow(illegalStateException);
    File file = new File("pathname1");
    target = new WalletAppKit(paramsMock2, ScriptType.P2PKH, keyChainGroupStructureMock, file, "wallet");
    ...
    doReturn(null).when(listenableCompletableFutureMock).whenComplete((BiConsumer) any());
    doNothing().when(downloadListenerMock).await();
    //Act Statement(s)
    target.startUp();
    //Assert statement(s)
    ...
    public void closeTest() {
         ...
         */
        //Arrange Statement(s)
        NetworkParameters paramsMock2 = mock(NetworkParameters.class, "BitcoinNetwork.TESTNET");
        doReturn(BitcoinNetwork.MAINNET).when(paramsMock2).network();
        KeyChainGroupStructure keyChainGroupStructureMock = mock(KeyChainGroupStructure.class, "KeyChainGroupStructure.DEFAULT");
        target = new WalletAppKit(paramsMock2, ScriptType.P2PKH, keyChainGroupStructureMock, (File) null, "wallet");
        autoCloseableMocks = MockitoAnnotations.openMocks(this);
        //Act Statement(s)
        target.close();
        //Assert statement(s)
